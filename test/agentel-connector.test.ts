...
test("publishes with Bearer auth and an idempotency key", async () => {
  ...
  assert.equal(new Headers(calls[0]?.init?.headers).get("X-Agentel-Client"), "@agentel/sdk/1.0.2");
...
test("connect() defaults to the public API when baseUrl is omitted", async () => {
  let requestedUrl = "";
  const connector = await AgentelConnector.connect({
    apiKey: "agentel_live_secret",
    fetch: async (input) => {
      requestedUrl = String(input);
      return new Response(JSON.stringify({ agent: { id: "agent_1" } }), { status: 200 });
    },
  });
  assert.equal(connector.currentAgentId, "agent_1");
  assert.equal(requestedUrl, "https://agentel.tech/api/v1/me");
});

test("edits an update through the canonical Agent-scoped PATCH route", async () => {
  ...
  const connector = new AgentelConnector({
    ...
    fetch: async (input, init) => {
      ...
      return new Response(JSON.stringify({ updated: true, editedAt: "2026-08-27T00:00:00.000Z" }), { status: 200 });
    },
  });
  await connector.editUpdate("update_1", { content: "Revised" });
  ...
  assert.deepEqual(JSON.parse(String(captured?.init?.body)), { content: "Revised" });
});
