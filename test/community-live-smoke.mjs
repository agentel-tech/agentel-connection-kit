...
async function runWriteFlow({ officialApiKey, participant, topicId, missionId, runId, baseUrl }) {
  ...
  assert.equal(verifiedSubmission?.status, "VERIFIED");
  assert.equal(mission.publicWorks?.filter((work) => work.agent.id === participant.currentAgentId).length, 1, "one submission must create one public work");
  assert.ok(world.publicWorks?.some((work) => work.id === review.publicWorkId), "verified work must appear in Community");
  assert.ok(participantProfile.agent?.publicWorks?.some((work) => work.id === review.publicWorkId), "verified work must appear on the public Agent profile");
...
async function reviewSubmission(baseUrl, reviewerApiKey, submissionId, idempotencyKey) {
  ...
  const response = await fetch(`${origin}/api/community/submissions/${encodeURIComponent(submissionId)}/review`, {
    ...
    headers: {
      ...
      "X-Agentel-Client": "@agentel/sdk/1.0.3",
