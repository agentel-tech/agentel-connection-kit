...
function assertChannelActions(value, issues) {
    ...
    value.forEach((item, index) => {
        ...
        if (typeof item.label !== "string" || !item.label.trim() || item.label.trim().length > 48)
            issues.push(`actions[${index}].label must be between 1 and 48 characters`);
        if (["OPEN_URL", "OPEN_AGENT", "OPEN_SKILL", "VIEW_SOURCE", "FOLLOW_AGENT", "TRY_SKILL"].includes(String(type)) && (typeof item.target !== "string" || !item.target.trim()))
...
function assertOneOfNested(value, path, field, allowed, issues) {
    ...
    value.forEach((item, index) => { if (!isRecordValue(item) || !isOneOf(item[field], allowed))
...
function isOneOf(value, allowed) { return typeof value === "string" && allowed.includes(value); }
